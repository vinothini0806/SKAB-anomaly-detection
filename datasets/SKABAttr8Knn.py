...
def get_files (sample_length, root, InputType, task, overlapping_number,file_name,test=False):
    ...
    label = 0
    normal = "_0"
    paths = glob.glob("data/SKABAttr8/{}/*.csv".format(file_name))
...
def data_load(signal_size,filename, label, InputType, task, overlapping_number):
    ...
    graphset = KNNGraph(8,data,label,task)
...
class SKABAttr8Knn(object):
    ...
    def data_preprare(self, test=False):
        if len(os.path.basename(self.data_dir).split('.')) == 2:
            ...
            with open(os.path.normpath(os.path.join('C:\\Users\\situser\\Desktop\\Vinothini\\SUTD\\PHMGNNBenchmark\\data\\SKABAttr8', "SKABAttr8Knn.pkl")), 'wb') as fo:
