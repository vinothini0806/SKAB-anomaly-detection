...
def get_files(sample_length, root, InputType, task,test=False):
    ...
    paths = glob.glob("data/SKABM/*.csv")
...
def data_load(signal_size,filename, label, InputType, task):
    ...
    graphset = RadiusGraph(8,data,label,task)
    print("length of graphset",len(graphset),len(graphset[0]))
    return graphset



class SKABMRadius(object):
    num_classes = 2
    ...
    def data_preprare(self, test=False):
        if len(os.path.basename(self.data_dir).split('.')) == 2:
            ...
            with open(os.path.normpath(os.path.join('C:\\Users\\situser\\Desktop\\Vinothini\\SUTD\\PHMGNNBenchmark\\data\\SKABM', "SKABMRadius.pkl")), 'wb') as fo:
