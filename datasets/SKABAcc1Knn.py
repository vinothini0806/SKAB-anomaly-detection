...
def get_files (sample_length, root, InputType, task, overlapping_number,file_name,test=False):
    ...
    label = 0
    normal = "_0"
    paths = glob.glob("data/SKABAcc1/{}/*.csv".format(file_name))
...
class SKABAcc1Knn(object):
    ...
    def data_preprare(self, test=False):
        if len(os.path.basename(self.data_dir).split('.')) == 2:
            ...
            with open(os.path.normpath(os.path.join('C:\\Users\\situser\\Desktop\\Vinothini\\SUTD\\PHMGNNBenchmark\\data\\SKABAcc1', "SKABAcc1Knn.pkl")), 'wb') as fo:
